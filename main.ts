...
function render() {
    let isLightArr: number[][] = [],
        tmpLight: number[][] = [];
    for (let i = 0; i < height; ++i) {
        for (let j = 0; j < width; ++j) {
            if (isLight[i][j]) isLightArr.push([i, j]);
        }
        tmpLight.push(Array<number>(width).fill(0));
    }
    for (let i = 0; i < height; ++i)
        for (let j = 0; j < width; ++j) {
...
function placeLight(x: number, y: number) {
    if (isLight[y][x]) {
        isLight[y][x] = false;
...
function reset() {
    lightCount = 0;
    for (let i = 0; i < height; ++i) for (let j = 0; j < width; ++j) isLight[i][j] = false;
    render();
}
...
for (let i = 0; i < height; ++i) {
    blocks.push(
        Array.from({ length: width }, (_element, index) => {
    ...
    isLight.push(Array<Boolean>(width).fill(false));
